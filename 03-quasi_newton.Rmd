...
\begin{align}
  ...
  &\text{subject to} \quad H = H^\top, \quad H_{s_k} = y_k.
\end{align}
...
Firstly we will define an objective function to test the BFGS on, we will use the Rosenbrock function.
...
we can now compare that to the `optim` functions implementation of 'Nelder-Mead', which is a derivative free optimisation algorithm

```{r}
optim(x_0, f, method = 'Nelder-Mead')$counts[1]
```
