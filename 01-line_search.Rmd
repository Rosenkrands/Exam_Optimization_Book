# Line search
A line serach algorithm chooses a direction $p_k$ and searches along this direction from the current iterate $x_k$ for a new iterate with a lower function value.
...
**Sufficient decrease**:
This condition ensure that the step insures a sufficient decrease in the objective function and can be formulated as
\begin{align}
...
```{r, include = F}
plot.data <- cbind('Fixed-step' = basic_gd(output = TRUE), 'Strong-Wolfe' = strong_wolfe_gd(output = TRUE, f, g, c(-19,3.6)), 'Iteration' = seq(1,100000,1))
library(tidyverse)
...
ggplot(plot.data.long) + geom_point(aes(x = Iteration, y = f, color = Type)) + theme_minimal()
```
